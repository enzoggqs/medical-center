...
// Read doctors
secretaryRoutes.get("/secretary/doctors", authSecretary, async (request, response) => {
    console.log(request.userType)
    if(request.userType != 'secretary')
        return response.status(400).json("Permission denied")
    const doctors = await prisma.doctor.findMany()
    return response.status(200).json({doctors: doctors, user: request.userId});
})

// Update doctor
secretaryRoutes.put("/secretary/doctors", authSecretary, async (request, response) => {
    if(request.userType != 'secretary')
        return response.status(400).json("Permission denied")
...
// Delete Doctor
secretaryRoutes.delete("/secretary/doctor/:id", authSecretary, async (request, response) => {
    if(request.userType != 'secretary')
        return response.status(400).json("Permission denied")
    const {id} = request.params;
    const intId = parseInt(id);
...
// Create appointment
secretaryRoutes.post("/secretary/appointment", authSecretary, async (request, response) => {
    if(request.userType != 'secretary')
        return response.status(400).json("Permission denied")
...
// Read appointments
secretaryRoutes.get("/secretary/appointments", authSecretary, async (request, response) => {
    if(request.userType != 'secretary')
        return response.status(400).json("Permission denied")
...
// Update appointment
secretaryRoutes.put("/secretary/appointment", authSecretary, async (request, response) => {
    if(request.userType != 'secretary')
        return response.status(400).json("Permission denied")
