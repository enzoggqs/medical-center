...
authenticationRoute.post("/secretary", async (request, response) => {
    ...
    const secretaryAlreadyExist = await prisma.secretary.findUnique({ where: { email }});
    const doctorAlreadyExist = await prisma.doctor.findUnique({ where: { email }});
...
// Register Doctor
authenticationRoute.post("/doctors", authSecretary, async (request, response) => {
...
// Secretary Login
authenticationRoute.post('/authenticate/secretary', async(request, response) => {
    const {email, password} = request.body;

    if(!email)
        return response.status(400).send("Email field not filled")
    ...
    console.log(secretaryAlreadyExist)
